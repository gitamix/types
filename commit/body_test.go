...
func TestBody_Empty(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "non-empty body",
			b:    commit.NewBody([]byte("This is a commit body.")),
			want: false,
		},
		{
...
func TestBody_String(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			b:    commit.NewBody([]byte("Special chars: !@#$%^&*()_+")),
			want: "Special chars: !@#$%^&*()_+",
		},
		{
			name: "body with unicode characters",
			b:    commit.NewBody([]byte("Unicode test: 測試")),
			want: "Unicode test: 測試",
		},
		{
			name: "body with tabs and newlines",
			b:    commit.NewBody([]byte("Line 1\twith tab\nLine 2")),
